...
network = ControlPowerFlow.ParserPWF.parse_file("data/5bus_csca.pwf", add_control_data = true)
network_contingency = ControlPowerFlow.ParserPWF.parse_file("data/5bus_csca_cont.pwf", add_control_data = true)
...
int_gen = ["22", "13"]
load_scenarios = Dict()
load_scenarios_cont = Dict()
gen_scenarios = Dict()
gen_scenarios_cont = Dict()
...
for (l, v) in network_contingency["load"]
    load_scenarios_cont[l] = network_contingency["load"][l]["pd"] 
    load_scenarios_cont[l] *= l in int_load ? profile.*mixed_seasonality(h) : ones(size(profile))
end
...
for (g, v) in network_contingency["gen"]
    gen_scenarios_cont[g] = network_contingency["gen"][g]["pg"]
    gen_scenarios_cont[g] *= g in int_gen ? profile.*mixed_seasonality(h) : ones(size(profile))
end
...
for s in 1:1, t in 1:steps_ahead  
    ...
    if t in contingency_range[s]
        data = deepcopy(network_contingency)
        apply_scenario!(data, load_scenarios_cont, gen_scenarios_cont, t, s)
...
plot(pm_results["vm"]["3"][:, 1], ylims = lims, label = "", size = (900, 600), color = :1)
plot!(pm_results["vm"]["4"][:, 1], ylims = lims, label = "", color = :2)
plot!(pm_results["vm"]["5"][:, 1], ylims = lims, label = "", color = :3)
...
plot!(ct_results["vm"]["3"][:, 1], ylims = lims, label = "", size = (900, 600), color = :1, line = :dash)
plot!(ct_results["vm"]["4"][:, 1], ylims = lims, label = "", color = :2, line = :dash)
plot!(ct_results["vm"]["5"][:, 1], ylims = lims, label = "", color = :3, line = :dash)
